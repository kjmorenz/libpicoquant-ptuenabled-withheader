...
int main(int argc, char* argv[])
{
  ...
  if((argc<3)||(argc>3))
  {
   printf("usage: ht2demo infile outfile\n");
   printf("infile is a Unified TTTR ptu file (binary)\n");
   printf("outfile is ASCII\n");
   getch(); //changed from _getch, hopefully should work the same since now in linux
   exit(-1);
  }
  ...
  do
  {
    ...
    Result = fread( &TagHead, 1, sizeof(TagHead) ,fpin);//KEEP
    ...
    switch (TagHead.Typ)
    {
      ...
      case tyAnsiString:
        ...
        //fwprintf(fpout, L"%s", WideBuffer);//changed since fwprintf is a super old function which no longer works
        free(WideBuffer);
        break;
            case tyBinaryBlob:
  ...
  while((strncmp(TagHead.Ident, FileTagEnd, sizeof(FileTagEnd))));//KEEP
  fprintf(fpout, "\n-----------------------\n");
// End Header loading
