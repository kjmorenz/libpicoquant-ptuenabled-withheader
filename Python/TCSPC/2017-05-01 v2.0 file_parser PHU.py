...
'''This script parses .PHU files from the Time/Pico/Hydra-Harp series of TCSPC modules.
...
print ("Acquisition Time: %d (ps)" %AcqTime)
...
It just works.'''
'''2015-05-03 Update: The loop below is much better, leaving this here
in case a brute force method is needed'''
...
#######################################################################
Counts=[]
seek="HistResDscr_DataOffset("
dictKey = "HistResDscr_HistogramBins("
for i in range(int(NumberOfCurves)):
    seek=seek+str(i+1) + ")"
    dictKey = dictKey + str(i+1) + ")"
    f.seek(emp[seek],0)
    Counts.append(fread(f,emp[dictKey],'uint32'))
    dictKey = "HistResDscr_HistogramBins("
    seek="HistResDscr_DataOffset("
Peak1=np.amax(Counts[0])
Peak2=np.amax(Counts[1])
############################################################################
...
This will (for the time being) print them on different figures'''
empty=[]
for i in range(len(Counts[0])):
    empty.append(i)
for i in range(int(NumberOfCurves)):
    plt.semilogy(empty,Counts[i])
