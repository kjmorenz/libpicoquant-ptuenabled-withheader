...
from regexp import regexpLogFiles
...
DoParam=0   ###Make sure to check this flag
...
if LoadSkip==False:
    ...
    if ExpLogFiles==[]:
        ...
        logDetails = regexpLogFiles(PathD, ExpLogFiles)
        MainData = []
        for line in logDetails:
                MainData.append(Curve(line
                                      ,PathD))
...
if DoParam==1 or DoParam==0 or DoParam==9:
  ...
  for j in range(len(MainData)):
        if MainData[j].counts == []:
                print('Curve MainData index ' + str(j+1) + ' is null, so it cannot be plotted.')
        elif MainData[j].peak <= 0:
                print('Curve at MainData index ' + str(j) + ' has no positive values, so it cannot be plotted.')
        else:
                #ColourSet[j]= 1
                plt.semilogy(MainData[j].counts)
                plt.xlabel('Run ' + str(MainData[j].run))
